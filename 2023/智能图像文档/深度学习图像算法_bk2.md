...
## 图片标注
#### 标注工具
- 选择Labelme进行标注:label_names.txt的结构：\_\_ignore__ \_background_ 接着是自定义的各种类别。使用Labelme的方法，pip install labelme，进入标注文件夹，在anaconda prompt用指令：
```shell
labelme Images --labels label_names.txt --nodata --validatelabel exact --config "{shift_auto_shape_color: -2}"
```

## 语义分割
- 实时性的语义分割网络，基于高效率的模型bisenet，RTX3090实测下来视频处理速度可达50FPS。数据集放在opendata上，且正依托项目不断进行扩充。
...
## torch转tensorrt
- tensorrt是nvidia基于自家的显卡推出的神经网络加速SDK，对外闭源（怪不得Linus要友好地竖中指），只提供了一些接口，方便用户自己写算子。将Bisenet转为onnx再转为tensorrt后，处理单张图片的速度由1.5FPS升至50FPS，但处理视频的速度由50FPS降至10FPS；且tensorrt的fp32推理结果肉眼可见地差，因此仍部署cuda版本到gpuservice中。但基于这份工作，我们之后可以用onnx格式去适配其他环境如ios等。
